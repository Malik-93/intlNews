...
import { closeFCM, codepush_sync, CODE_PUSH_OPTIONS, initFCM, setGlobals } from './src/utils';
...
const Icon = ({ element: { channalIcon = '', id = 1, url = '' }, selected = 1, onChannelPress }) => {
  return <TouchableOpacity onPress={() => onChannelPress(url, id)} style={[{ height: ICON_SIZE + extra, width: ICON_SIZE + extra, borderRadius: (ICON_SIZE + extra) / 2, marginHorizontal: 5, justifyContent: "center", alignItems: "center" }, id == selected ? { borderWidth: 1.5, borderColor: '#addfed' } : {}]}>
  ...
  const getSetData = () => {
    fetch(baseUrl + '/api/permissions', {
      ...
      .then(data => {
        ...
        setTimeout(() => {
          setShowAd(true);
        }, data.permissions.interval || (2 * 60000));
      })
      .catch(() => { })
      .finally(() => { })
  }
  ...
  return (
    <SafeAreaView style={backgroundStyle}>
      ...
      <View style={styles.container}>
        ...
        <WebView
          ref={web_view_ref}
          source={{ uri }} onNavigationStateChange={({ loading, url, canGoBack, canGoForward, }) => {
            // console.log('canGoBack', canGoBack);
            setLoading(loading)
          }} />
      </View>
      {
        showAd && <RewardedInterstitialAd />
      }
      <BannerAd />
    </SafeAreaView>
  );
